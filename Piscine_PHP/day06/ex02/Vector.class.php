<?php
	...
	class Vector {
		...
		static function doc() {
			...
			if ($str = file_get_contents('../ex02/Vector.doc.txt')) {
				echo "$str";
			}
			else {
		...
		public function magnitude() {
			...
			$magn = (float)sqrt(($this->_x - $orig->x) ** 2 +
								($this->_y - $orig->y) ** 2 +
								($this->_z - $orig->z) ** 2);
		...
		public function normalize() {
			...
			$norm = new Vector(array('dest' => new Vertex(
				array(	'x' => $this->_x / $len,
						'y' => $this->_y / $len,
						'z' => $this->_z / $len
													))));
		...
		public function add(Vector $rhs) {
			...
			$add = new Vector(array('dest' => new Vertex(
		...
		public function sub(Vector $rhs) {
			...
			$sub = new Vector(array('dest' => new Vertex(
				array(	'x' => $this->_x - $rhs->_x,
						'y' => $this->_y - $rhs->_y,
						'z' => $this->_z - $rhs->_z
														))));
		...
		public function opposite() {
			...
			$opp = new Vector(array('dest' => new Vertex(
				array(	'x' => $this->_x * (-1),
						'y' => $this->_y * (-1),
						'z' => $this->_z * (-1)
													))));
		...
		public function scalarProduct($k) {
			...
			$scl = new Vector(array('dest' => new Vertex(
				array(	'x' => $this->_x * $k,
						'y' => $this->_y * $k,
						'z' => $this->_z * $k
												))));
		...
		public function crossProduct(Vector $rhs) {
			...
			$cross = new Vector(array('dest' => new Vertex(
